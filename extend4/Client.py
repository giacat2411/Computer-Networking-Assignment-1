...
class Client:
	...
	def createWidgets(self):
		...
		# Create time
		self.time_label  = Label(self.master, text=self.time_display,bg="white",fg="black")
		...
		while True:
			try:
				...
				if data:
					...
					self.time_label.config(text = str(round((self.end_time - self.start_time + self.time_display)*100)/100) + "s")
					if current_Frame_Number > self.frameNbr:
						self.frameNbr = current_Frame_Number
						self.updateMovie(self.writeFrame(rtpPacket.getPayload()))
			except:
				self.time_display += self.end_time - self.start_time
